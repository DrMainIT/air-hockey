...
from air_hockey_challenge.environments.data.custom import __file__ as env_path
...
class AirHockeyBase(MuJoCo):
    ...
    def __init__(self, gamma=0.99, horizon=500, timestep=1 / 1000., n_intermediate_steps=20, n_substeps=1,
        ...
        if 1 <= self.n_agents <= 2:
            scene = os.path.join(os.path.dirname(os.path.abspath(env_path)), "custom.xml")

            action_spec += ["custom_robot_1/joint_1", "custom_robot_1/joint_2"]

            observation_spec += [("robot_1/joint_1_pos", "custom_robot_1/joint_1", ObservationType.JOINT_POS),
                                 ("robot_1/joint_2_pos", "custom_robot_1/joint_2", ObservationType.JOINT_POS),
                                 ("robot_1/joint_1_vel", "custom_robot_1/joint_1", ObservationType.JOINT_VEL),
                                 ("robot_1/joint_2_vel", "custom_robot_1/joint_2", ObservationType.JOINT_VEL)]

            additional_data += [("robot_1/ee_pos", "custom_robot_1/body_ee", ObservationType.BODY_POS),
                                ("robot_1/ee_vel", "custom_robot_1/body_ee", ObservationType.BODY_VEL)]

            collision_spec += [("robot_1/ee", ["custom_robot_1/ee"])]
        else:
        ...
        self.env_info['joint_pos_ids'] = [6, 7]
        ...
        robot_model = mujoco.MjModel.from_xml_path(
            os.path.join(os.path.dirname(os.path.abspath(env_path)), "stepper.xml"))
        robot_model.body('custom_robot_1/body_1').pos = np.zeros(3)
        robot_data = mujoco.MjData(robot_model)

        # Add env_info that requires mujoco models
        self.env_info['dt'] = self.dt
        self.env_info["robot"]["joint_pos_limit"] = np.array(
            [self._model.joint(f"custom_robot_1/joint_{i + 1}").range for i in range(2)]).T
        ...
        frame_T = np.eye(4)
        temp = np.zeros((9, 1))
        mujoco.mju_quat2Mat(temp, self._model.body("custom_robot_1/body_1").quat)
        frame_T[:3, :3] = temp.reshape(3, 3)
        frame_T[:3, 3] = self._model.body("custom_robot_1/body_1").pos
    ...
    def _modify_mdp_info(self, mdp_info):
        obs_low = np.array([0, -1, -np.pi, -20., -20., -100,
                            *np.array([self._model.joint(f"custom_robot_1/joint_{i + 1}").range[0]
                                       for i in range(self.env_info['robot']['n_joints'])]),
                            *self.env_info['robot']['joint_vel_limit'][0]])
        obs_high = np.array([3.02, 1, np.pi, 20., 20., 100,
                             *np.array([self._model.joint(f"custom_robot_1/joint_{i + 1}").range[1]
                                        for i in range(self.env_info['robot']['n_joints'])]),
