...
from air_hockey_challenge.environments.custom import AirHockeyBase
...
class AirHockeySingle(AirHockeyBase):
    ...
    def setup(self, state=None):
        for i in range(2):
            self._data.joint("custom_robot_1/joint_" + str(i + 1)).qpos = self.init_state[i]
            self.q_pos_prev[i] = self.init_state[i]
            self.q_vel_prev[i] = self._data.joint("custom_robot_1/joint_" + str(i + 1)).qvel
